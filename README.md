...
**The Chrome extension** (in `extension/`, load unpacked via `chrome://extensions`) authors in your everyday browser: click the toolbar action, pick elements on the page, edit timing/easing/holds in the side panel, preview the motion in real time, export the config. **● Record** arms record mode: a banner appears on the page, your pointer movement, clicks and scrolling are captured until you press ESC, and the take lands as one `record` step. (The preview replays a recording's scroll, traces a cursor dot, and emulates the hover under it with the same untrusted-input approximation hover steps use; click and drag effects are render-only, where they run as trusted input.) Two honesty features matter:
